...
// group currently in the queue is pushed back, and bumps the new-checkin
// minimum-wait floor by the same amount — both used when the course is
// running slower than the configured rate.
export async function POST() {
  ...
  const extraFloorMinutes = parseFloat(settings.add_time_total_minutes ?? '0')

  const newSmallRate = smallRate + ADD_MINUTES
  const newLargeRate = largeRate + ADD_MINUTES
  const newExtraFloorMinutes = extraFloorMinutes + ADD_MINUTES
  ...
  const { error: writeError } = await supabase.from('settings').upsert([
    ...
    { key: 'add_time_total_minutes', value: String(newExtraFloorMinutes) },
  ])
  ...
  return NextResponse.json({
    ...
    add_time_total_minutes: newExtraFloorMinutes,
  })
}
